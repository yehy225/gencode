...
<head>
	<title><%=${className}Mo.TABLE_ALIAS%> 维护</title>
	<link href="<@jspEl 'ctx'/>/statics/css/managerSystem.css" rel="stylesheet" type="text/css" />
	<script type="text/javascript" src="<@jspEl 'ctx'/>/statics/script/My97DatePicker/WdatePicker.js"></script>
	<script type="text/javascript" src="<@jspEl 'ctx'/>/statics/script/jquery-1.7.2.js"></script>
	<script type="text/javascript" src="<@jspEl 'ctx'/>/statics/script/jquery.form.js"></script>
</head>
<body>
<div class="title01">
	<h2>搜索<span class="zksq"><!-- <a href="" title="收起展开"><img alt="展开收起按钮" height="7" src="<@jspEl 'ctx'/>/images/zksq.png" width="13" /></a> --></span></h2>
</div>
<form id="queryForm" name="queryForm" action="<c:url value="${actionBasePath}/list.${actionExtension}"/>" method="post" style="display: inline;">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table00">
		...
		<tr>	
			...
			<td class="txtr" style="width: 100px"><%=${className}Mo.ALIAS_${column.constantName}%>：</td>		
			<td>
	...
	<br></br>
	<div class="title01">
	<h2><%=${className}Mo.TABLE_ALIAS%>列表</h2>
</div>
		<div class="groupbtn">
		...
		<div>
			...
			<thead>
			 ...
			 <#list table.columns as column>
				<#if !column.htmlHidden>
				<th><%=${className}Mo.ALIAS_${column.constantName}%><div style="width: 65px; height: 1px; overflow: hidden;"></div></th>
				</#if>
			</#list>
			...
			<tr>
				<td><input name="ids" type="checkbox" value="<@jspEl 'entry.id'/>"/></td>
			 ...
			 <#list table.columns as column>
				...
				<td>
						<a href="#" onclick="show('<@jspEl 'entry.id'/>')">详情</a>
						<a href="#" onclick="edit('<@jspEl 'entry.id'/>')">修改</a>
						<a href="#" onclick="del('<@jspEl 'entry.id'/>')">删除</a>
				</td>
			</tr>
			</c:forEach>
			</table>
     	 <yehy:page url="<@jspEl 'ctx'/>${actionBasePath}/list.${actionExtension}" id="logForm" submitForm="queryForm" joy="true"/>
</div>

<script type="text/javascript">
function show(id){
	window.open('<@jspEl 'ctx'/>${actionBasePath}/show.${actionExtension}?id='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
function edit(id){
	window.open('<@jspEl 'ctx'/>${actionBasePath}/edit.${actionExtension}?id='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
...
$(function(){
	...
	$('#btnedit').click(function(){
		...
		window.open('<@jspEl 'ctx'/>${actionBasePath}/edit.${actionExtension}?id='+$id.val(),'','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
	
	$('#btnadd').click(function(){
		window.open('<@jspEl 'ctx'/>${actionBasePath}/create.${actionExtension}','','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
...
function v_deleteItems(ids){
	if(ids.length>0 && confirm("确定要删除吗？")){
		$.post( "<@jspEl 'ctx'/>${actionBasePath}/delete.${actionExtension}", {"items":ids.join(',')}, 
				function(data){
					if(data=='1'){
						alert('删除成功');
						$('#queryForm').submit();
